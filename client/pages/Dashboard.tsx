...
import { Home, Calendar, FileText, Search, PanelLeft, AlertTriangle } from "lucide-react";
...
const LabCard = ({ lab }: { lab: Lab }) => (
  <Card className="flex items-center p-4 space-x-4 overflow-hidden w-full">
    <img src={lab.image} alt={lab.name} className="w-20 h-20 md:w-24 md:h-24 object-cover rounded-md" />
    <div className="flex-grow">
      <p className="text-xs text-gray-500">{lab.location}</p>
      <h3 className="text-base md:text-lg font-bold text-gray-900">{lab.name}</h3>
      <p className="text-sm text-gray-600 mt-1">Status Kelas :</p>
      <StatusBadge status={lab.status} />
    </div>
    <Button asChild size="sm" className="bg-blue-600 hover:bg-blue-700 text-white shrink-0 self-end">
...
const DashboardContent = () => {
  ...
  const renderContent = () => {
    if (loading) {
        return Array.from({ length: 4 }).map((_, i) => <LabCardSkeleton key={i} />)
    }
    ...
    if (labs.length === 0) {
        return (
            <div className="col-span-full flex flex-col items-center justify-center text-center py-10">
                ...
                <p className="text-gray-600">Belum ada data laboratorium yang tersedia.</p>
            </div>
        )
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans flex">
      ...
      <SidebarInset className="flex-1 pb-20 md:pb-0">
        ...
        <header className="bg-white md:bg-transparent p-4 shadow-sm md:shadow-none sticky top-0 z-10">
          <div className="flex justify-between items-center">
            ...
            <h1 className="font-bold text-lg md:hidden">Pilih Laboratorium</h1>
        ...
        {/* Content Body */}
        <main className="p-4">
          <h1 className="text-2xl font-bold text-gray-800 mb-4 hidden md:block">Pilih Laboratorium</h1>
          {/* Search for Mobile */}
      ...
      <footer className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-3 shadow-[0_-1px_10px_rgba(0,0,0,0.05)]">
          <button onClick={toggleSidebar} className="flex flex-col items-center gap-1 text-gray-500">
