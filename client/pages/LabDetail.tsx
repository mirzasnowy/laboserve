    ...
    <div className="relative min-h-screen w-full bg-gray-50 flex flex-col font-sans">
        ...
        <main className="flex-grow p-4 lg:p-8">
            <div className="max-w-6xl mx-auto">
                <img src={lab.image} alt={lab.name} className="w-full h-48 md:h-64 lg:h-80 object-cover rounded-xl shadow-lg"/>
                
                <div className="mt-6 flex justify-between items-start">
                    <div>
                        <p className="text-sm text-gray-500">{lab.location}</p>
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900">{lab.name}</h2>
                    </div>
                ...
                <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <ul className="text-gray-700 list-disc list-inside space-y-1 bg-white p-6 rounded-lg shadow-sm">
                            {Object.entries(lab.specifications).map(([key, value]) => (
                                <li key={key} className="text-md">{key}: <span className="font-semibold">{value}</span></li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div>
                        <h3 className="text-xl font-semibold text-gray-800 mb-3">Jadwal Terdekat</h3>
                        <Card className="flex items-center p-3 gap-3 overflow-hidden shadow-sm">
                            <img src={mockSchedule.image} alt={mockSchedule.className} className="w-20 h-20 object-cover rounded-md" />
                            <div className="flex-grow text-sm">
                                <p className="text-xs text-gray-500">Lantai 3 - Gedung Fasilkom</p>
                                <h4 className="font-bold text-gray-900">{mockSchedule.className}</h4>
        ...
        <footer className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm p-4 border-t">
            <Button className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-lg rounded-full">Reservasi</Button>
        </footer>

        {/* Reservasi Button for Desktop - could be placed differently, e.g., sticky on the side */}
        <div className="hidden md:block fixed bottom-8 right-8">
             <Button className="h-14 px-8 bg-blue-600 hover:bg-blue-700 text-lg rounded-full shadow-lg">Reservasi</Button>
        </div>
    </div>
  );
}
