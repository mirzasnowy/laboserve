...
const SetPasswordStep = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      toast({ title: "Error", description: "Gagal membuat password.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Sebagai langkah keamanan tambahan, silakan buat password untuk akun Anda.</CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Silakan isi informasi berikut untuk menyelesaikan pendaftaran.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
...
export default function Onboarding() {
  ...
  const isDetailsSet = (profile.type === 'dosen' && !!profile.nidn) || (profile.type === 'mahasiswa' && !!profile.kelas);

  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        {!isPasswordSet ? <SetPasswordStep /> : !isDetailsSet ? <SetDetailsStep /> : <p>Menyelesaikan...</p>}
      </div>
    </div>
  );
}